...
flowchart LR
    ...
    findings -- No --> gate{Disposition gate}
    findings -- Yes, repair available --> correct[Correct once]
    correct --> validate2[Deterministic validation]
    validate2 --> gate
    findings -- Yes, no repair available --> gate

    gate -- ready --> publish[Publish final briefing]
    gate -- review_required --> quarantine[Quarantine preview]
    gate -- rejected --> quarantine
    gate -- no_result --> quarantine
    ...
    validate2 -. blinded artifacts .-> judges[Semantic judge + grounding judges]
...
**Claim grounding is sampled, not asserted.** Verifying that prose is entailed by its source needs a semantic judge, so the deterministic checker does not pretend to settle it. Its figure, quotation, and length checks are review signals, all at WARN. A figure missing from the cited excerpts is cross-checked against item-level corpus evidence: when the exact figure appears in an uncited item with at least three shared title terms and 60% overlap with the shorter title, the checker emits a non-blocking `figure_supported_elsewhere` quality note instead of claiming the figure is unsupported. The conservative title threshold prevents a common number elsewhere in a large corpus from laundering an unrelated claim. The committed reference pair evaluates clean under those heuristics, but that result does not establish semantic entailment; run the checker and review the evidence for each new output.
