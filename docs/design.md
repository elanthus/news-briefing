...
[`run_briefing.py`](../run_briefing.py) is an orchestration boundary rather than another agent prompt. It owns the sequence fetch → project → generate → validate → repair → correct → finalize, while provider adapters implement one `ModelProvider` protocol. The primary workflow remains Python-standard-library-only: OpenRouter uses `urllib`, and the Claude Code and Codex adapters launch their installed commands directly.
