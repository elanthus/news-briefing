...
|---|---:|---|---|---:|---:|---:|
| Codex CLI `gpt-5.6-terra` | 2 | 3 errors, 0 warnings | 0 errors, 1 warning | 106.650s | 512 | subscription; unavailable |
...
All four completed runs ended `review_required`: their corrected or first accepted candidates remained corpus-bound and contract-accepted, but the current multi-axis outcome policy quarantines evidence warnings instead of publishing them. Terra retained one `unsupported_figure` warning. Hy3 retained three `unsupported_figure` and two `claim_exceeds_evidence` warnings. DeepSeek retained one of each. Gemini retained one `exclusion_log_short` and two `claim_exceeds_evidence` warnings. The corpus had the same four recorded coverage gaps as the Claude structured-output follow-up.
