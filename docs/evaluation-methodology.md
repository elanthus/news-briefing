...
How the evaluator's suites are scored, and what those scores may and may not be used to claim. Read this before quoting a number from [`evaluator/`](../evaluator/README.md), [Portfolio v2](results/portfolio-v2.md), or [Parity v2](results/parity-v2.md): the sections below define each score family's denominator, the boundary between deterministic and human judgment, and the limitations that rule out a single composite reliability score.
...
Current generation uses only the production two-pass path and current corpus schema. Direct-Markdown runs and obsolete corpus formats are retained as historical evidence, reproducible from their recorded source revision. Public bundle verification still checks frozen hashes and regenerates reports without model calls. Current fixture metadata was refreshed without changing evidence text or labels; this does not constitute a new model or human review.

## Scope and threat model

The evaluator measures two separate systems. The 81-case offline suite measures deterministic briefing checks and feed parsing. The 55-case generation suite measures model behavior on 22 utility cases and 33 indirect prompt-injection attacks (21 primary cases plus 12 position/count ablation replicates); five attacks also create clean twins at run time. [Portfolio v2](results/portfolio-v2.md) ran on the evaluator's direct-Markdown path and measures the weaker citation contract. [Parity v1](results/parity-v1.md) used the production two-pass contracts but skipped the deterministic repair step that production runs before a model correction, so it remains historical evidence. [Parity v2](results/parity-v2.md) reran the matrix through the corrected production-parity path. These score families have different units and denominators and are never combined into one reliability score.
...
The promotion rule requires the end-to-end utility delta to meet the improvement threshold in [`regression-policy.json`](../evaluator/regression-policy.json). Attack success uses non-inferiority: the upper bound of the paired authored-case-cluster bootstrap confidence interval for the candidate-minus-baseline attack-success delta must remain below the policy's margin. Comparison output records the SHA-256 hash of the policy bytes so the decision thresholds are attributable to an exact policy version.
...
The parity-v1 numbers predate the repair-path correction and remain historical evidence. Parity v2 is the live-model rerun on the corrected path; its two malformed-JSON provider errors and lack of completed human semantic or grounding review remain explicit limitations rather than being converted into scored failures or successes.
...
The workflow decrypts each day's retained diagnostics artifact and, for every `ready` run, builds a blinded grounding packet from that run's own finalized structured candidate and its frozen, position-scoped selected evidence (`evaluator/production_grounding.py`, reusing `redact_destinations()` and `redact_opaque_references()` from `agent_runner/output.py`). The judge model — called through the same `ModelProvider`/evaluator adapter path used elsewhere, offered no tools — never receives a citation destination or an opaque `citation_`/`item_` handle, matching the boundary the production generation models are held to. A structural repair that drops or reorders an included topic after its evidence was frozen breaks that position alignment for the affected run; such a run is skipped from that week's measurement rather than risk pairing prose with the wrong evidence.

The result is published as an **unverified machine grounding rate**: the share of reviewed topics with no automated grounding error, alongside its topic count and a 95% Wilson interval, in the committed [weekly log](results/grounding-monitor.md). This is the same "automated model judgment, not human labels or independent human approval" disclosure used by `evaluator/grounding_machine_review.py` elsewhere in this document, applied to production output instead of a benchmark manifest. Per-topic verdicts and rationale stay in the workflow's encrypted uploaded artifact; only the rate and the count are public.

This monitor cannot be used to claim any single day's briefing was verified: it runs after publication, on a sample, through one automated judge with no independent audit-agreement guarantee beyond the stratified double-review comparison already reported alongside the rate, and it can only detect the same class of unsupported, reversed, or strengthened claims the human-review rubric targets. It never blocks, delays, or retroactively withdraws a publication; a bad week's rate is a monitoring signal to investigate, not a rollback trigger.
