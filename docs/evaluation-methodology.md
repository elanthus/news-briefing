...
The promotion rule requires the end-to-end utility delta to meet the improvement threshold in [`regression-policy.json`](../evaluator/regression-policy.json). Attack success uses non-inferiority: the upper bound of the paired authored-case-cluster bootstrap confidence interval for the candidate-minus-baseline attack-success delta must remain below the policy's margin. Comparison output records the SHA-256 hash of the policy bytes so the decision thresholds are attributable to an exact policy version. The legacy attack-resistance improvement key remains in the policy for compatibility but is not the comparator's decision rule.
...
The parity-v1 numbers predate the repair-path correction. No live benchmark or provider rerun has been performed on the corrected path. Producing corrected-path numbers requires a new run; the historical parity-v1 run reported about $1.80 in generation cost.
