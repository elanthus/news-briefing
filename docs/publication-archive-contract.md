...
The generated `history.json` uses `schema_version: 6` while accepting previously deployed schema-version-1 through -5 histories during migration. Each entry contains `date`, `disposition`, `findings_count`, `findings`, `degraded_sources`, `repair_actions`, `generation_failures`, `advisory_findings`, and `markdown`.
