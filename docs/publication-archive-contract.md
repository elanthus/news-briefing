...
The text-free `corpus-storage.json` marker records that a successful Pages build uses encrypted operational storage rather than legacy public corpus files. During the one-time transition, scheduled and multi-date runs download all thirteen retained legacy dates into a staging directory, validate them, and copy them into private storage only as a complete set; a partial migration fails before deployment. A validated manual single-date repair may continue from a partial legacy download only when its requested corpus is present (or when the requested date is today and will be fetched fresh). That exception seeds private storage with every still-recoverable corpus but regenerates only the requested date. After the marker has been published, an expired or missing Actions artifact starts a fresh window instead of repeatedly attempting legacy migration. Today's corpus is still fetched fresh, while unavailable historical backfill dates are skipped.
...
The generated `history.json` uses `schema_version: 7` while accepting previously deployed schema-version-1 through -6 histories during migration. Each entry contains `date`, `disposition`, `findings_count`, `findings`, `degraded_sources`, `repair_actions`, `generation_failures`, `advisory_findings`, `provenance`, and `markdown`.
