...
For Hacker News items whose article (`url`) and HN discussion (`discussion`) resolve to different destinations, include both links, each on its own 🔗 line. If they resolve to the same destination (as with an HN self-post), print that URL only once. Do not print Hacker News points or comment counts; they are mutable snapshots that can be stale by reading time.
🔗 [article URL]
🔗 HN: [discussion URL]
