...
class BuildSiteTests(unittest.TestCase):
    ...
    def test_schema_v6_history_without_provenance_still_loads(self) -> None:
        # Older history entries predate issue #174 and carry no provenance
        # object; they must still validate and render.
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            empty = root / "empty"
            empty.mkdir()
            build_site(empty, root / "site", prior_history=legacy_history)
            history = json.loads((root / "site/history.json").read_text(encoding="utf-8"))
            self.assertEqual(history["entries"][0]["provenance"], None)
            report = (root / "site/reports/2026-08-19.html").read_text(encoding="utf-8")
            self.assertNotIn("provenance", report)
    ...
    def _write_sidecar(
        ...
        *,
        ...
        # optional field is requested, fill in the rest of the highest
        # requested version's set rather than writing a partial shape no real
        # producer emits.
        ...
        elif repair_actions is not None or advisory_findings is not None:
            payload["repair_actions"] = repair_actions or []
            payload["generation_failures"] = []
            payload["advisory_findings"] = advisory_findings or []
