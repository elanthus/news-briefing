...
This historical report was regenerated after the 2026-08-13 redundancy trim (54 → 49 checker/feed cases), blinded model review and owner adjudication of those 49 cases, model review and owner adjudication of the six subsequent coverage additions, the UTF-32 parser fix, and model review and owner adjudication of the final 24 paired heuristic-claim cases and two UTF-32 regressions, by running:

```bash
python3 -m evaluator checker
```

No external model was called while regenerating these metrics. Nemotron Ultra had reviewed the original 49 cases (38 exact agreements, 11 owner-adjudicated disagreements, one final label change). GLM 5.2 then reviewed the six coverage additions (3 exact agreements and 3 owner-adjudicated disagreements, no final label changes), followed by the final 26-case packet (23 exact agreements and 3 owner-adjudicated disagreements, two final label changes). These LLM reviews helped bootstrap the repository; they were not independent human review, and full human review is recommended before production use. This historical report predates the 2026-08-25 repair of `structure-overfilled` and `selection-category-ambiguity`; both repaired fixtures completed renewed exact-agreement model review on 2026-08-26, so all 81 current cases have model review. See `label_provenance` in `evaluator/fixtures/checker-cases.json`. Live model, prompt-version, correction, attack, grounding, latency, and cost rows have **0 trials / not run** in this baseline; `python3 -m evaluator run` produces those rows from actual provider calls. The offline `baseline` provider (empty/echo/compliant) is the one exception — see "Offline generation-harness baselines" below, which reports real numbers from a zero-cost, zero-credential run.
